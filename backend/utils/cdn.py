...
# domain
cdn_domain = "https://cdn.<INSERT DOMAIN URL HERE>"
...
def _get_cf_url(api_type: str) -> str:
    ...
    match api_type:
        case "image":
            return "https://api.cloudflare.com/client/v4/accounts/{account_id}/images/v1".format(
                CF_ID
            )
        case "image delete":
            return "https://api/cloudflare.com/client/v4/accounts/{account_id}/images/v1/{image_id}".format(
                CF_ID
            )
        case "file":
            return "https://api.cloudflare.com/client/v4/accounts/{account_id}/".format(
                CF_ID
            )
    ...
    for row in data:
        if row["type"] == "img":
            ...
            if all([img_url.scheme, img_url.netloc]):
                ...
                if res.status_code == 200:
                    file = FileStorage(BytesIO(res.content))

                    image_name = "image_" + uuid.uuid4().hex + ".png"

                    # cf_url, img_id = upload_image(file, image_name, "full")
                    # row["url"] = cf_url
                    # row["id"] = img_id

                    # development code till CDN runs
                    current_dir = Path(__file__).resolve().parents[1]
                    fp = current_dir / "cdn" / image_name
                    file.save(fp)
                    row["url"] = str(fp)
                    row["id"] = img_id
                    img_id += 1
                ...
                file = FileStorage(BytesIO(bytes(row["url"])))
                image_name = "image_" + uuid.uuid4().hex + ".png"
                # cf_url, img_id = upload_image(file, image_name, "full")
                # row["url"] = cf_url
                # row["id"] = img_id
        ...
        if row["type"] == "img":
            ...
            if all([img_url.scheme, img_url.netloc]):
                ...
                if res.status_code == 200:
                    ...
                    cf_url, img_id = patch_image(file, image_name, img_id, "full")
                    row["url"] = cf_url
                    row["id"] = img_id
